        ...
        <div className="w-full max-w-md bg-white rounded-xl p-8 shadow-lg">
          ...
          <div>
            ...
            <div className="space-y-3">
              <button 
                onClick={() => !socialLoading && handleSocialLogin('google')}
                disabled={socialLoading}
                className={`w-full flex items-center justify-center gap-2 h-11 rounded-md border border-gray-300 bg-white text-sm font-medium transition-colors ${socialLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50'}`}
                type="button"
              >
                ...
                <span>{socialLoading ? "Signing in..." : "Log in with Google"}</span>
              </button>
              
              <button 
                onClick={() => !socialLoading && handleSocialLogin('facebook')}
                disabled={socialLoading}
                className={`w-full flex items-center justify-center gap-2 h-11 rounded-md border border-gray-300 bg-white text-sm font-medium transition-colors ${socialLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50'}`}
                type="button"
              >
                ...
                <span>{socialLoading ? "Signing in..." : "Log in with Facebook"}</span>
              </button>
              
              <button 
                onClick={() => !socialLoading && handleSocialLogin('apple')}
                disabled={socialLoading}
                className={`w-full flex items-center justify-center gap-2 h-11 rounded-md border border-gray-300 bg-white text-sm font-medium transition-colors ${socialLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-50'}`}
                type="button"
              >
                ...
                <span>{socialLoading ? "Signing in..." : "Log in with Apple"}</span>
              </button>
            </div>
